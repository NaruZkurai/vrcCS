namespace NZK{  public static partial class S{
   ...
   *   and S.P.C the same again for Path.Combine.
   *
   * FILE NAME: the leaves are named S.P.<letter>.cs - the member's FIRST
   * letter only, so the name states the SHAPE (String, Path, then which
   * member) rather than repeating the member name the file already contains.
   *
