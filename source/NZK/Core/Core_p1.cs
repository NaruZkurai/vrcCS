...
public static partial class Core {
  ...
  public static void FixZeroScaleAnimations()
  {
    ...
    if (clip == null)
    {
      NZK.E.Dd("Not an Animation Clip",
    ...
    if (FixAnimationClipScale(clip))
    {
      ...
      NZK.E.Dd("Animation Scale Fix",
          "Fixed zero scale values in: " + clip.name, "OK");
    }
    else
    {
      NZK.E.Dd("No Fixes Needed",
          "No zero-scale animation data found in: " + clip.name, "OK");
    }
  }
  ...
  public static void FixZeroScaleAnimationsAsset()
  {
    ...
    System.Collections.Generic.List<string> fixedClips =
      new System.Collections.Generic.List<string>();
    int skipped = 0;
    foreach (string guid in guids)
    {
      ...
      if (FixAnimationClipScale(clip)) fixedClips.Add(clip.name);
    }
    if (fixedClips.Count > 0)
    {
      NZK.AD.R();
      NZK.E.Dd("Animation Scale Fix",
          "Fixed zero scale values in:\n" + string.Join("\n", fixedClips), "OK");
    }
    else
    {
      NZK.E.Dd("No Fixes Needed",
          "No zero-scale animation data found in the selected clips.", "OK");
    }
  }
