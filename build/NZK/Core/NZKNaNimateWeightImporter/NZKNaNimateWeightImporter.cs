...
public sealed class NZKNaNimateWeightImporter : UnityEditor.AssetPostprocessor
{
     ...
     * exactly NZKNaNimateWeightNormalizer.NaNimateWeight (1e-07) and all other
    ...
    public void OnPreprocessModel()
    {
        ...
        /* ORDER IS LOAD-BEARING: skinWeights = Custom MUST BE ASSIGNED FIRST.
         *
         * Unity validates these as they are set and warns - not errors - when
         * they arrive while the mode is still Standard:
         *
         *   ModelImporter.maxBonesPerVertex is ignored unless
         *     ModelImporter.skinWeights is set to ModelImporterSkinWeights.Custom
         *   ModelImporter.minBoneWeight is ignored unless ...
         *
         * The assignments are DISCARDED in that state, so setting the mode last
         * meant both the four-bone cap and the zero threshold were silently
         * lost on every import.  Same class of failure as the original 255: the
         * value is written, the warning is easy to read past because it is a
         * warning and not an error, and the importer keeps its previous value.
         *
         * Measured on this project: with the mode last, the .meta kept
         * skinWeightsMode 1 and the two settings below it had no effect.
         */
        importer.skinWeights = UnityEditor.ModelImporterSkinWeights.Custom;
        /* Now the settings that Custom makes meaningful. */
    ...
    public void OnPostprocessModel(UnityEngine.GameObject root)
    {
        ...
        for (int r = 0; r < renderers.Length; r++)
        {
            ...
            NZKNaNimateWeightNormalizer.Result result =
                new NZKNaNimateWeightNormalizer.Result();
            string[] boneNames = NZKNaNimateWeightNormalizer.BoneNamesOf(renderer);
            ...
            try
            {
                NZKNaNimateWeightNormalizer.Normalize(probe, boneNames, result);
            }
            finally
            {
            ...
            UnityEngine.Debug.Log(NZK.S.NZKNaNimatePrefix("Would pin " + result.verticesAffected +
                                  " vertex/vertices to " + NZKNaNimateWeightNormalizer.NaNimateWeight +
                                  " on " + renderer.name +
    ...
    public static void NormalizeSelected()
    {
        ...
        for (int p = 0; p < paths.Length; p++)
        {
            ...
            string folder = System.IO.Path.GetDirectoryName(path)
                .Replace('\\', '/');
            NZKNaNimateWeightNormalizer.Result result =
                NZKNaNimateWeightNormalizer.NormalizeModel(
