...
public static partial class MA {
    ...
    { Report report = new Report();
       ...
       * (`BEFORE bones=228` on the second run).  Detect our own output: every
       * bone we create is named `<group>` or `<group>_<i>` and lives under a
       * `NZK_NanimBuffer_<n>` transform, so a renderer that already carries a
       * buffer object has already been processed. */
      ...
      { UnityEngine.Transform t = existing[b];
        ...
        if (p != null && p.name.StartsWith("NZK_NanimBuffer",System.StringComparison.Ordinal))
        { report.MeshesSkipped = 1;
          report.error = "already processed: bone '" + t.name + "' sits under '" + p.name + "'";
          return report; } }
      ...
      { System.String shapeName = mesh.GetBlendShapeName(si);
        ...
        Group g = new Group { name = bone.name };
    ...
    { Report report = new Report();
       ...
       * carrying a NZK buffer bone has been processed and must not be stacked on. */
      UnityEngine.Transform[] existing = renderer.bones;
      for (System.Int32 b = 0; b < existing.Length; b++)
      { UnityEngine.Transform t = existing[b];
        if (t == null) continue;
