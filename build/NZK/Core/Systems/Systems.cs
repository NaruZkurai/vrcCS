...
public static partial class Systems {
 /* //TODO: meshes.cs - right click split meshes by material,save to storage + master file (same system as NaNimate) */
  ...
  { /* ── Only scan components in the current scene (not project assets) ── */
    var checkedGos = new System.Collections.Generic.HashSet<UnityEngine.GameObject>();
    var toScan = new System.Collections.Generic.List<UnityEngine.GameObject>();
    foreach (var root in UnityEngine.SceneManagement.SceneManager.GetActiveScene().GetRootGameObjects()) toScan.Add(root);
    toScan.Add(target);
    foreach (var s in sources) if (s != null) toScan.Add(s);
    var sourceSet = new System.Collections.Generic.HashSet<UnityEngine.GameObject>(sources);
    for (int i = 0; i < toScan.Count; i++)
    { var go = toScan[i]; if (go == null || !checkedGos.Add(go)) continue;
      foreach (UnityEngine.Transform c in go.transform) toScan.Add(c.gameObject);
      var comps = go.GetComponents<UnityEngine.Component>();
      foreach (var comp in comps)
      { if (comp == null) continue;
        var sobj = new UnityEditor.SerializedObject(comp);
        var prop = sobj.GetIterator(); System.Boolean changed = false;
        while (prop.NextVisible(true))
        { if (prop.propertyType == UnityEditor.SerializedPropertyType.ObjectReference && prop.objectReferenceValue is UnityEngine.GameObject goRef)
          { if (sourceSet.Contains(goRef)) { prop.objectReferenceValue = target; changed = true; } } }
        if (changed) sobj.ApplyModifiedProperties(); } } }
  ...
  { foreach (var src in sources)
    { if (src == null || src == target) continue;
      ...
      foreach (var c in comps)
      { if (c == null || c is UnityEngine.Transform) continue;
        var destType = c.GetType(); var existing = target.GetComponent(destType);
        if (existing == null) { var nc = target.AddComponent(destType); UnityEditor.Undo.RegisterCreatedObjectUndo(nc,"Add merged component"); UnityEditor.EditorUtility.CopySerialized(c,nc); }
        else { UnityEditor.Undo.RecordObject(existing,"Update merged component"); UnityEditor.EditorUtility.CopySerialized(c,existing); UnityEditor.EditorUtility.SetDirty(existing); }
  ...
  { /* ── Pre-validate: filter nulls, fallback to active selection ── */
    ...
    if (cat1 == "Mesh" && cat2 == "Merge") { Meshes.MergeSelectedObjectsFromSelection(); Systems.MenuSuccess("Merge Meshes","Meshes merged."); return; }
