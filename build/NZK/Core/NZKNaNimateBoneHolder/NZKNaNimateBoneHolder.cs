...
[UnityEditor.InitializeOnLoad]
 public partial class NZKNaNimateBoneHolder : UnityEngine.MonoBehaviour {
    ...
    { System.String why;
      ...
      for (System.Int32 i = 0; i < SourceModels.Length; i++)
      { if (SourceModels[i] == null) continue;
        ...
        if (NZK.S.HasOIC(p, ".blend") || NZK.S.HasOIC(p, ".fbx") ||
          ...
          models.Add(p); }
      if (models.Count == 0)
      { UnityEngine.Debug.LogWarning("[NZK BoneHolder] SourceModels is empty or holds no model asset. " +
                                     "Select the .blend/.fbx with the avatar selected and click " +
                                     "'Capture Source Models From Selection'.");
        return; }
    ...
    { UnityEngine.GameObject target = ResolveTarget();
      ...
      if (found.Count == 0)
        UnityEngine.Debug.LogWarning("[NZK BoneHolder] no source model found. If Generate has already run, " +
                                     "the renderers point at generated meshes and this can no longer be captured - " +
                                     "set SourceModels by dragging the .blend in by hand.");
      else
