...
public static partial class MeshImport {
    ...
    /** <summary>Every serialised name the influence cap is known to travel
     *  under, newest first.
     *
     *  MEASURED ON THIS PROJECT'S UNITY, and the two names are NOT aliases.
     *  Dumped straight off the importer for Nemasis.blend (Unity 2022.3.22f1):
     *
     *      skinWeightsMode    [Integer] = 1     <- the Inspector's dropdown
     *      maxBonesPerVertex  [Integer] = 255   <- the cap; 255 == Unlimited
     *      minBoneWeight      [Float]   = 0
     *      optimizeBones      [Boolean] = False
     *
     *  Both exist AT THE SAME TIME and both are enforced.  Writing only one is
     *  the bug this list used to cause: the old code walked this list, matched
     *  `maxBonesPerVertex` first, set it to 4 and reported Changed=1 - while
     *  `skinWeightsMode` stayed at its own value and the importer kept obeying
     *  Unlimited.  The Inspector looked right and the upload was still wrong.
     *
     *  ORDER MATTERS AND IS NOT ALPHABETICAL.  `skinWeightsMode` is the mode
     *  switch and goes FIRST, because Unity re-derives the cap when the mode
     *  changes: writing the cap first lets the mode change overwrite it.  The
     *  mode value for the four-bone case is the enum's 4, which coincides with
     *  the plain count, so one constant serves both.</summary> */
    public static readonly System.String[] MaxBonesNames = { "skinWeightsMode", "maxBonesPerVertex" };
     ...
     *  influence is discarded at import.
     *
     *  THE NAME IS `minBoneWeight`, AND IT WAS `weightThreshold` HERE.
     *  `weightThreshold` DOES NOT EXIST on this importer - verified by
     *  FindProperty returning null - so the probe matched nothing and the cutoff
     *  was never touched, while the code read as though it were handled.  A
     *  setting looked up under the wrong name fails exactly like one that is
     *  absent, which is the worst way for it to fail.
     *
     *  AND IT STILL CANNOT BE SET, recorded here so the next reader does not
     *  repeat the attempt.  Unity 2022.3 forces this to 0.001 and refuses every
     *  write path: SerializedObject, the native typed setter, SetDirty +
     *  WriteImportSettingsIfDirty, ImportAsset(ForceUpdate), a .meta reserialize,
     *  and editing the .meta BY HAND - writing 0.01 reads back as 0.001, so it is
     *  not even a floor, the field is ignored outright.  The read-back in
     *  ForceFour reports that honestly instead of claiming success. */
    public const System.String WeightThresholdProperty = "minBoneWeight";
    /** <summary>The first property that exists on this importer, or null.
     *
     *  Returns the SerializedProperty alongside the NAME it was found under,
     *  because the two names mean different things even though they accept the
     *  same int - the caller logs which one applied, and a log that says
     *  "maxBonesPerVertex" when the importer actually speaks
     *  `skinWeightsMode` would hide the very mismatch this exists to catch.
     *  `isEnum` distinguishes them so an out-of-range write can be reported
     *  rather than silently clamped by Unity.</summary> */
    public static UnityEditor.SerializedProperty FindFirst(
      UnityEditor.SerializedObject so,System.String[] names,out System.String found)
    { found = null;
      if (so == null || NZK.B.mpty.t(names)) return null;
      for (System.Int32 i = 0; i < names.Length; i++)
      { UnityEditor.SerializedProperty p = so.FindProperty(names[i]);
        if (p != null) { found = names[i]; return p; } }
      return null; }
    ...
    { ImportReport report = new ImportReport();
      ...
      { if (!NZK.S.Has(path)) continue;
        ...
        /* The cap is written through EVERY name that exists, not one chosen up
           front - see MaxBonesNames.  `strip` and `threshold` are separate
           settings and are probed normally; a version that lacks one of them
           still gets the others applied rather than being skipped wholesale. */
        ...
        System.Boolean anyCap = false;
        for (System.Int32 ni = 0; ni < MaxBonesNames.Length; ni++)
          if (so.FindProperty(MaxBonesNames[ni]) != null) { anyCap = true; break; }
        if (!anyCap && strip == null && threshold == null)
        { /* None of the settings exists on this importer - the model format may
             not support skin weights at all (.obj), or this Unity version names
             them differently again.  Reported WITH the property names actually
             present, because "unsupported" alone leaves the reader unable to
             tell those two cases apart and therefore unable to fix either one. */
          report.Unsupported++;
          NZK.E.C.w(67,path + " - tried " + System.String.Join(", ",MaxBonesNames) +
                        ", " + OptimizeBonesProperty +
        ...
        /* EVERY name in MaxBonesNames that this importer exposes gets written,
           not just the first one found.  They are separate serialised fields
           (skinWeightsMode AND maxBonesPerVertex, both present on 2022.3) and
           both are enforced, so capping only the first left the second at 255
           (Unlimited) and the importer kept obeying it while the code reported
           success.  Writing each one that exists is what makes the outcome the
           same on every Unity version instead of only on the one it was
           measured on. */
        System.Int32 capsWritten = 0;
        for (System.Int32 ni = 0; ni < MaxBonesNames.Length; ni++)
        { UnityEditor.SerializedProperty p = so.FindProperty(MaxBonesNames[ni]);
          if (p == null) continue;
          so.ApplyModifiedPropertiesWithoutUndo();
          if (p.intValue == MaxInfluences) { capsWritten++; continue; }
          p.intValue = MaxInfluences;
          changed = true;
          capsWritten++; }
        if (capsWritten == 0 && strip == null && threshold == null)
        { report.Unsupported++;
          NZK.E.C.w(65,path + " - none of " + System.String.Join(", ",MaxBonesNames) +
                        " exists on this importer; has: " + PropertyNames(so));
          continue; }
        ...
        so.ApplyModifiedPropertiesWithoutUndo();
        /* READ BACK, and fail loudly.  A write Unity accepted but discarded
           (wrong property for the version, or a mode that re-derives the cap)
           is indistinguishable from success unless the value is read again -
           and that silence is exactly how the importer sat at 255 through a
           "Changed=1" report. */
        for (System.Int32 ni = 0; ni < MaxBonesNames.Length; ni++)
        { UnityEditor.SerializedProperty p = so.FindProperty(MaxBonesNames[ni]);
          if (p == null) continue;
          if (p.intValue != MaxInfluences)
          { report.Unsupported++;
            NZK.E.C.w(65,path + " - " + MaxBonesNames[ni] + " would not accept " +
                          MaxInfluences + " (still " + p.intValue +
                          "); this importer does not honour the cap");
            continue; } }
      ...
      /* One batched reimport at the end rather than one per asset: each reimport
         ...
         THE BATCH IS THE ONLY REIMPORT, AND IT ONLY CONTAINS WHAT CHANGED.
         `reimport` is appended to solely on the `changed` path above, so a model
         already at the cap never enters it.  That is what keeps this call cheap
         on the second and every later run - which matters because the holder
         re-runs on EVERY import, so an unconditional reimport here would make
         the pipeline re-import its own inputs forever at full pipeline cost.
         THE TRAILING Refresh() IS GONE, and removing it is not an optimisation
         detail.  StopAssetEditing() already flushes and refreshes the pending
         imports; a second Refresh() on top of it starts ANOTHER asset scan, and
         that scan fires OnPostprocessAllAssets again - the same self-triggering
         path the holder's import guard exists to absorb.  It bought nothing and
         cost a full project rescan per run. */
      if (reimport.Count > 0)
    ...
    /** <summary>One serialised INT setting off an importer, or a marker when
     *  this Unity version does not expose it.
     *
     *  Exists beside FloatSetting because the two influence-cap properties are
     *  BOTH ints and both live at once on 2022.3 - the diagnostic has to show
     *  them side by side, since a correct-looking `skinWeightsMode=4` next to a
     *  stale `maxBonesPerVertex=255` is the exact state that made a broken
     *  import look fixed.</summary> */
    static System.String IntSetting(UnityEditor.SerializedObject so,System.String name)
    { if (so == null) return "?";
      UnityEditor.SerializedProperty p = so.FindProperty(name);
      return p == null ? "(absent)" : p.intValue.ToString(); }
    ...
    { System.Collections.Generic.HashSet<System.String> paths = PathsOf(objs);
      ...
      { if (!NZK.S.Has(path)) continue;
        ...
        /* The cap is reported under the NAME it was found by, and its raw value
           is included.  A bare "maxBonesPerVertex=4" on a 2022.3 importer would
           be a lie - the property does not exist there and the number would
           come back "(absent)"; printing the found name makes
           `skinWeightsMode=255` (Unlimited, uncapped) legible as the failure it
           is instead of looking like a count. */
        System.String capFound = null;
        UnityEditor.SerializedProperty capProp = FindFirst(so,MaxBonesNames,out capFound);
        NZK.E.C.d(100,"model=" + System.IO.Path.GetFileName(path) +
                      " cap=" + (capFound == null ? "(absent, tried " +
                                                System.String.Join("/",MaxBonesNames) + ")"
                                                : capFound + "=" + capProp.intValue) +
                      " skinWeightsMode=" + IntSetting(so,"skinWeightsMode") +
                      " maxBonesPerVertex=" + IntSetting(so,"maxBonesPerVertex") +
