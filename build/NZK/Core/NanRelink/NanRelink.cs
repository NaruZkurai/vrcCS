...
public static class NanRelink
  {
     ...
     *  for a mesh is the one whose name matches the mesh (or the mesh's source
     *  object); the toggle generator names them that way on purpose.
     *
     *  Matching order, most specific first:
     *    1. exact name equality with the mesh name
     *    2. equality ignoring the _NZKRelink suffix this tool adds
     *    3. bone contains the mesh name, or the mesh name contains the bone stem
     *  Falls back to FirstNanimBone only when nothing matches, so a mesh whose
     *  bone is oddly named is still bound rather than left dead.</summary> */
    public static System.Int32 NanimBoneFor(UnityEngine.Transform[] bones,System.String meshName)
    { if (NZK.B.mpty.t(bones)) return -1;
      if (!NZK.S.Has(meshName))
      { /* No name to match on: any nanimation bone is better than none. */
        return FirstNanimBone(bones); }
      System.String want = StripSuffix(meshName);
      /* The bone is named "NaNimate " + the source object name, so comparing
         against the PREFIX-STRIPPED bone name is what actually lines them up. */
      for (System.Int32 i = 0; i < bones.Length; i++)
      ...
      /* Looser: either name contains the other.  Handles the generator suffixing
         a group ("NaNimate GRC" vs "NaNimate GRC chains"). */
      for (System.Int32 i = 0; i < bones.Length; i++)
      { if (bones[i] == null) continue;
        System.String bn = bones[i].name;
        if (!IsNanimBone(bn)) continue;
        System.String stem = StripNanimPrefix(bn);
        if (!NZK.S.Has(stem)) continue;
        if (NZK.S.HasOIC(want,stem) || NZK.S.HasOIC(stem,want)) return i; }
      return FirstNanimBone(bones); }
     ...
     *
     *  nanimBone is the bind-bone index to use; pass -1 when the renderer has no
     *  nanimation bone, in which case the vertex is returned untouched rather
     *  than silently mangled.</summary> */
    ...
    { System.Single near = NZK.Core.Meshes.Vars.Consts.DefaultNearZeroVertexWeight;
      /* No nanimation bone on this renderer.  Return as-is: fabricating a
         binding would point the vertex at a bone that does not exist. */
      if (nanimBone < 0) return w;
      ...
      /* Keep the THREE heaviest.  Only three slots remain beside the handle, so
         ...
         cannot fit - never one of the strong ones.  Selection sort over at most
      ...
      if (total <= 0f)
        return new UnityEngine.BoneWeight { boneIndex0 = nanimBone,weight0 = 1f,
                                            ...
                                            boneIndex2 = 0,weight2 = 0f,
                                            boneIndex3 = 0,weight3 = 0f };
      /* Slot 0 is the handle.  Slots 1-3 hold the surviving real influences,
         scaled so the four weights sum to 1 without ever dividing the handle. */
     ...
     *  crosses from source to scene.</summary> */
    ...
    { System.Single[] v = new System.Single[4];
      ...
      { System.Single w = WeightAt(src,i);
        ...
        if (si < 0 || srcBones == null || si >= srcBones.Length || srcBones[si] == null) continue;
        if (target.TryGetValue(srcBones[si].name,out System.Int32 t)) b[i] = t; }
